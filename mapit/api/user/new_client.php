	  ...
	  if( filter_var($email, FILTER_VALIDATE_EMAIL) ){
	      ...
	      if(User::is_user($email)){
	      	...
	      	if(UserInstitution::exists($user['user_id'], $institution_id)){
	      		...
	      		if(UserInstitution::save($user['user_id'], $institution_id, CLIENT) and MapUser::assign_to_maps($map_ids, $user['user_id'], $institution_id) ){
	        ...
	        if(User::create_client($institution_id, $user_id, $full_name, $email, $pass, $phone) and MapUser::assign_to_maps($map_ids, $user_id, $institution_id) ){
